...
// Новая функция для красивого форматирования количества кругов на русском языке
function formatLaps(laps: number) {
  const formatted = laps % 1 === 0 ? laps.toString() : parseFloat(laps.toFixed(2)).toString();
  const n = Math.floor(laps) % 100;
  const n1 = n % 10;
  
  if (laps % 1 !== 0) return `${formatted} круга`; // Дробные значения всегда "круга" (напр. 2.5 круга)
  if (n > 10 && n < 20) return `${formatted} кругов`;
  if (n1 > 1 && n1 < 5) return `${formatted} круга`;
  if (n1 === 1) return `${formatted} круг`;
  return `${formatted} кругов`;
}
...
export function RunningCalc() {
  ...
  // Добавлено поле laps в тип результата
  ...
  function calculate() {
    ...
    if (mode === "pace") {
      ...
      setResult({ 
        ...
        laps: formatLaps(laps) // Добавляем отформатированное количество кругов
      });
      setSplits(buildSplits(dist, calculatedPaceSecPerKm, splitStep));
    }
  }
      ...
      <div className="w-full relative z-10 " style={{ maxWidth: "400px " }}>
        ...
        {result && (
          <div className="mt-4 " style={{ backgroundColor: "#1A1A1A ", border: `1.5px solid ${gold}`, borderRadius: "16px ", padding: "16px 20px ", boxShadow: `0 4px 20px rgba(201,168,76,0.15)` }}>
            ...
            {/* Новое поле для отображения количества кругов */}
            {result.laps && (
              <div style={{ marginTop: "12px ", paddingTop: "12px ", borderTop: `1px solid ${goldBorder}`, fontSize: "14px ", color: "#A0A0A0 ", fontFamily: "'JetBrains Mono', monospace ", display: "flex ", alignItems: "center ", justifyContent: "space-between " }}>
                <span>Количество кругов</span>
                <span style={{ color: gold, fontWeight: 700, fontSize: "16px " }}>{result.laps}</span>
              </div>
            )}
          </div>
        )}
